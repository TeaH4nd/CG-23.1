...
/**
 * Initial matrix scale.
 * @type {Number}
 */
var currentScale = 1;

/**
 * Maximal size of the square.
 * @type {Number}
 */
const MAX_SIZE = 3

/**
 * Minimal size of the square.
 * @type {Number}
 */
const MIN_SIZE = 0.5
...
function handleKeyPress(event) {
    ...
    //     break;
      case "+":
        if(currentScale <= MAX_SIZE) currentScale+=0.1;
        break;
      case "-":
        if (currentScale >= MIN_SIZE) currentScale -= 0.1;
        break;
      default:
        return;
    }
        ...
        function outOfBounds(){
            ...
            for(let i =0;i<4; i++){
                ...
                if (mypositon[0] < -wsize / 2 ){
                    changeMovement(cornersIndexes[i],mypositon,0);
                }
                else if(mypositon[0] > wsize / 2){
                    changeMovement(cornersIndexes[i],mypositon,1);
                }
                else if(mypositon[1] < -wsize / 2){
                    changeMovement(cornersIndexes[i],mypositon,3);
                }
                else if( mypositon[1] > wsize / 2){
                    changeMovement(cornersIndexes[i],mypositon,2);
                }
            }
        }
        //Processa qual tipo de colisão dependendo da selecionada
        //collisionType = 0 -left , 1- right , 2- up 3- down
        function changeMovement(collidedIndex,collisionPosition,collisonType){
