 ...
 * @type {Matrix4}
 */
var baseMatrix = new Matrix4().setTranslate(0, -13, 0).rotate(joint.base, 1, 0, 0);
var shaftMatrix = new Matrix4().setTranslate(0, -3, 0).rotate(joint.shaft, 0, 1, 0);
var generatorMatrix = new Matrix4().setTranslate(0, 10.5, 0).rotate(joint.generator, 0, 1, 0);
var rotorMatrix = new Matrix4().setTranslate(0, 0, 3.5).rotate(joint.rotor, 1, 0, 0);
var bladeMatrix = new Matrix4().setTranslate(0, 0, 0).rotate(joint.blade, 0, 0, 1);

var shaftMatrixLocal = new Matrix4().setScale(2, 25, 2);
var baseMatrixLocal = new Matrix4().setScale(6, 1.5, 6);
...
function handleKeyPress(event) {
  ...
  switch (ch) {
    ...
    case "g":
      joint.generator += 15;
      generatorMatrix.setTranslate(0, 10.5, 0).rotate(joint.generator, 0, 1, 0);
      break;
    case "G":
      joint.generator -= 15;
      generatorMatrix.setTranslate(0, 10.5, 0).rotate(joint.generator, 0, 1, 0);
...
window.addEventListener("load", (event) => {
  ...
  (function animate() {
    if(!isPaused) {
        rotorMatrix.rotate(bladeSpeed, 0, 0, 1);
    } else {
        rotorMatrix.rotate(0, 0, 0, 1);
    }
